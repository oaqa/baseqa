...
public abstract class AbstractInterpretedQuestionGenerator extends AbstractCandidateGenerator {
	...
	public void generateCandidates(JCas jcas)
	    ...
	    final String question = ((InputElement) BaseJCasHelper.getAnnotation(jcas, InputElement.type))
	            .getQuestion();
	    // do task
	    List<PlaceHolder> placeholders = generateInterpretedQuestion(question);
	    // save output
	    PlaceHolderList.storePlaceHolders(jcas, placeholders, false); 
	}

	  protected abstract List<PlaceHolder> generateInterpretedQuestion(final String question)
	          throws AnalysisEngineProcessException;
