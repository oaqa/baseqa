...
public abstract class AbstractFormulatedQueryGenerator extends
	...
	public void generateCandidates(JCas jcas)
	    ...
	    final String question = ((InputElement) BaseJCasHelper.getAnnotation(jcas, InputElement.type))
	            .getQuestion();
	    // do task
	    List<PlaceHolder> placeholders = generateFormulatedQuery(question);
	    // save output
	    PlaceHolderList.storePlaceHolders(jcas, placeholders, false); 
	}

	  protected abstract List<PlaceHolder> generateFormulatedQuery(final String question)
	          throws AnalysisEngineProcessException;
