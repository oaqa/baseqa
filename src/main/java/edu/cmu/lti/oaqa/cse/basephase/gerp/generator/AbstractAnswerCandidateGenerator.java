...
public abstract class AbstractAnswerCandidateGenerator extends
	...
	public void generateCandidates(JCas jcas)
	    ...
	    final String question = ((InputElement) BaseJCasHelper.getAnnotation(jcas, InputElement.type))
	            .getQuestion();
	    // do task
	    List<PlaceHolder> placeholders = generateAnswerCandidates(question);
	    // save output
	    PlaceHolderList.storePlaceHolders(jcas, placeholders, false); 
	}

	  protected abstract List<PlaceHolder> generateAnswerCandidates(final String question)
	          throws AnalysisEngineProcessException;
