...
public abstract class AbstractGatheredEvidenceGenerator extends
	...
	public void generateCandidates(JCas jcas)
	    ...
	    final String question = ((InputElement) BaseJCasHelper.getAnnotation(jcas, InputElement.type))
	            .getQuestion();
	    // do task
	    List<PlaceHolder> placeholders = generateGatheredEvidences(question);
	    // save output
	    PlaceHolderList.storePlaceHolders(jcas, placeholders, false); 
	}

	  protected abstract List<PlaceHolder> generateGatheredEvidences(final String question)
	          throws AnalysisEngineProcessException;
